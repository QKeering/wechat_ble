<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="modelList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设备型号编号" align="center" prop="id" />
      <el-table-column label="型号唯一标识符" align="center" prop="modelKey" />
      <el-table-column label="型号名称" align="center" prop="modelName" />
      <el-table-column label="设备版本" align="center" prop="deviceVersion" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="型号标识" prop="modelKey">
          <el-input v-model="form.modelKey" placeholder="请输入型号唯一标识符" />
        </el-form-item>
        <el-form-item label="型号名称" prop="modelName">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        modelKey: [
          { required: true, message: "型号唯一标识符不能为空", trigger: "blur" }
        ],
        modelName: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除设备型号编号为"' + ids + '"的数据项？').then(function() {
        return delModel(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('/admin/device/model/export', {
        ...this.queryParams
