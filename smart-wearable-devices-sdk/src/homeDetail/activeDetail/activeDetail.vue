...
onLoad(async (options) => {
  activeScore.value = options?.activeScore || 0;
  await getMotionDetailData();
});
onPageScroll((e) => {
...
<template>
  <view class="relative p-30">
    <uv-navbar @leftClick="leftClick" placeholder leftIcon="arrow-left" title="活动详情" :bgColor="scrollTop > 0 ? '#ffffff' : 'rgba(255, 255, 255, 0)'"></uv-navbar>
    ...
    <view class="content">
      ...
      <view class="health-card p-30 bg-white r-50 mt-40">
        <view class="flex ai-center jc-between mb-20">
          ...
          <text class="fs-28 t-979797">当前活动完成率</text>
        </view>
...
.bgcWrapper {
  ...
  height: 25vh;
  z-index: -1;
  background: linear-gradient(to bottom, #ffdd81, #f1f3f6);
}
.health-card {
