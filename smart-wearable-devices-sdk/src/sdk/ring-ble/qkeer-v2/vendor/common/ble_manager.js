...
class BleManager {
    ...
    return new Promise((resolve, reject) => {
      uni.getBLEDeviceServices({
        ...
        success: (res) => {
          ...
          // 直接查找目标服务UUID
          let targetService = res.services.find((service) => service.uuid === ble_config.UUID_SERVICE_TARGET);
  ...
  async getBLEDeviceCharacteristics(deviceId) {
    return new Promise((resolve, reject) => {
      uni.getBLEDeviceCharacteristics({
        ...
        success: (res) => {
          const targetCharacteristic = res.characteristics?.find((characteristic) => characteristic.uuid === ble_config.UUID_TARGET_NOTIFY);
  ...
  checkExistsDevice(device) {
    ...
    if (existsDevice) {
      if (existsDevice.RSSI > device.RSSI) return true;
