...
import { getActivityIntensityDetail } from '@/common/api/heatlthSummary';
import type { ActivityIntensityResp } from '@/types/api/healthSummary';
import { getSleepDurationHours, getSleepDurationMinutes } from '@/utils/utils.js';
import { cloneDeep } from 'lodash-es';
const echarts = require('../../static/echarts.min.js');
const activityIntensity = ref<ActivityIntensityResp>();
...
const getSecoundOption = () => {
  const testTrendChartData = [
    { weekLabel: '11月第1周', minutes: 211 },
    { weekLabel: '11月第2周', minutes: 332 },
    { weekLabel: '11月第3周', minutes: 456 },
    { weekLabel: '11月第4周', minutes: 222 },
    { weekLabel: '12月第1周', minutes: 123 },
    { weekLabel: '12月第2周', minutes: 444 },
    { weekLabel: '12月第3周', minutes: 111 }
  ];

  return createChartOption(
    sleepTimeOption,
    activityIntensity?.value?.durationChart || [],
    // testTrendChartData,
    (item) => Number(item.minutes),
    'steps'
  );
};
...
onLoad(async () => {
  const res = await getActivityIntensityDetail();
